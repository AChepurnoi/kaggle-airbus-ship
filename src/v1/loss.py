...
class LovaszSymLoss(nn.Module):
    def __init__(self):
        super().__init__()

    def forward(self, input, target):
        return symmetric_lovasz(input, target).mean()


class FocalLovasz(nn.Module):
    def __init__(self, gamma, weight=0.5):
        super().__init__()
        self.weight = weight
        self.focal = FocalLoss(gamma)
        self.lovasz = LovaszSymLoss()

    def forward(self, input, target):
        loss = self.weight * self.lovasz(input, target) + (1 - self.weight) * self.focal(input, target)
        return loss.mean()
...
def lovasz_loss(logits, labels, error_func, per_image=True, ignore=None):
    ...
    if per_image:
        loss = mean(
            lovasz_loss_flat(*flatten_binary_scores(log.unsqueeze(0), lab.unsqueeze(0), ignore), error_func=error_func)
